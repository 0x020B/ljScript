...
function startFengyi(){
	...
	for (var i=1;i<10;i++){
		...
		break;
	    if (npc.get("cmd"+i+"_name").match("禮包")!=null&&npc.get("cmd"+i+"_name").match("1元")==null&&npc.get("cmd"+i+"_name").match("兑换")==null)
...
function CheckIn(){ // 进入扬州
	...
	if (checkinstep==0){
	    ...
	    checkinstep++;
	    btnList["签到领奖"].innerText = '野外技能';
	}else if(checkinstep==4){
	    go('jh 4;n;n;n;n;n;n;n');
	    pushCmd('event_1_91604710');
	    pushCmd('go south');
	    pushCmd('go south');
	    go('s;w');
	    pushCmd('get_silver');
	    pushCmd('get_silver');
	    go('s;e;n;n;n;nw;s;s;w;n;n;n;e;n;n;w');
	    // pushCmd(); // 草垛
	    go('e;n;e');
	    pushCmd('huashan_luoyanya21_op1');
	    go('w;s;s;s;w;n;n;n;w');
	    for (var i=0; i<15; i++) {
		pushCmd('event_1_60189725');
	    }
	    pushCmd('go east');
	    pushCmd('go east');
	    pushCmd('go south');
	    pushCmd('look_npc huashan_huashan21');
	    pushCmd('event_1_17338310');
	    pushCmd('jh 18'); // 明教
	    pushCmd('go north');
	    ...
	    pushCmd('go north');
	    pushCmd('go north');
	    pushCmd('event_1_90080676');
	    pushCmd('event_1_56007071');
	    pushCmd('go northeast');
	    pushCmd('go north');
	    for (var i=0; i<5; i++) {
		pushCmd('event_1_27935051');
	    }
	    pushCmd('jh 18');
	    pushCmd('go east');
	    pushCmd('event_1_50855676');
	    pushCmd('jh 20'); // 古墓
	    pushCmd('go west');
	    pushCmd('go west');
	    pushCmd('go south');
	    ...
	    pushCmd('go east');
	    pushCmd('go east');
	    pushCmd('event_1_3723773');
	    pushCmd('get yin yaoshi');
	    pushCmd('go south');
	    pushCmd('give gumu_longnv');
	}
    }
}
var sendrequest=0;
...
function gumu(){
    var jhlist=g_obj_map.get("msg_jh_list").get("finish18");
    if (jhlist!=undefined&&jhlist!=0){
	    pushCmd('go west');
	    pushCmd('go west');
            pushCmd('go south');
            ...
            pushCmd('give gumu_longnv');
            for (var i=0; i<5; i++) {
                pushCmd('study gumu_yufeng-book');
            }
    }
...
(function (window) {
	...
	this.dispatchMessage = function(b) {
	    ...
	    if (type == "show_html_page" && msg.indexOf("知識問答第") > 0) {
		console.log(msg);
		if (msg.indexOf(" 回答正確！") > 0) {
		    pushCmd("question");
		    return;
		}
